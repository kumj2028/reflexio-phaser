...
interact with software. This interest in non-traditional game input began during
the author's undergraduate studies, where a system for playing Pong using
electroencephalography (EEG) signals recorded from a consumer headset was built
...
puzzle platformer originally written as an undergraduate course project in C\#
...
target size, which severely penalizes small or closely spaced targets. The
...
(click and cursor positions serve as ground-truth labels). No specialized
...
port preserves all published levels, the core reflection mechanic, player
...
via a class registry that maps XML tag names to constructors. The level XML
...
user gesture to maximize the playable area and lock landscape orientation on
mobile devices.

All menu items across every scene (main menu, level select, pause menu,
controls, and achievements) were made directly tappable using Phaser's
\texttt{setInteractive()} API. Tapping a menu item immediately activates it,
matching the interaction model mobile users expect from native apps. The level
select screen uses a two-tap convention: the first tap highlights a level and
shows its preview image, and a second tap on the already-selected thumbnail
starts the level. This prevents accidental launches from a stray touch while
remaining responsive to deliberate input.
...
However, the discretization (horizontal saccade $\to$ cycle horizontal lines;
...
Several limitations constrain the generalizability of these results. The
...
physics fidelity across all levels.
